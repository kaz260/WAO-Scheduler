...
// Get standard ambient temperature from the node's label.
func getAmbientInfo(nodeInfo *framework.NodeInfo) (float32, float32, error) {
	tempAmbMax, ambMaxOK := nodeInfo.Node().Labels[labelAmbientMax]
	tempAmbMin, ambMinOK := nodeInfo.Node().Labels[labelAmbientMin]
	if !ambMaxOK || !ambMinOK {
		klog.V(trace).Infof("%v : Ambient label is not defined. [%v : %v, %v : %v] ",
			nodeInfo.Node().Name, labelAmbientMax, ambMaxOK, labelAmbientMin, ambMinOK)
		return -1, -1, errors.New("Ambient label is not defined")
	}

	ambientMax, err := strconv.ParseFloat(tempAmbMax, 32)
	if err != nil {
		klog.V(trace).Infof("Convert to float64 failed. [%v : %v]", labelAmbientMax, tempAmbMax)
		return -1, -1, errors.New("Convert to float64 failed")
	}

	ambientMin, err := strconv.ParseFloat(tempAmbMin, 32)
	if err != nil {
		klog.V(trace).Infof("Convert to float64 failed. [%v : %v]", labelAmbientMin, tempAmbMin)
		return -1, -1, errors.New("Convert to float64 failed")
	}

	// The ambient max and min must not be the same because division by zero occurs in calcNormalizeTemperature.
	if (ambientMax - ambientMin) == 0 {
		klog.V(trace).Infof("%v : Do not set values of %v and %v the same", nodeInfo.Node().Name, labelAmbientMax, labelAmbientMin)
		return -1, -1, errors.New("division by zero")
	}
	return float32(ambientMax), float32(ambientMin), nil
}
...
func (oco *OsmoticComputingOptimizer) calcNormalizeTemperature(nodeName string, nodeInfo *framework.NodeInfo) (float32, float32, float32, error) {
	...
	ambientMax, ambientMin, err := getAmbientInfo(nodeInfo)
	if err != nil {
		return -1, -1, -1, err
	}
	...
	normalizedCPU1 := (oco.cpu1[nodeName] - ambientMin) / (ambientMax - ambientMin)
	normalizedCPU2 := (oco.cpu2[nodeName] - ambientMin) / (ambientMax - ambientMin)
